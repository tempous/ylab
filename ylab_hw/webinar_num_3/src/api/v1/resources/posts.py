...
@router.get(path="/{post_id}",response_model=PostModel,summary="Найти пост",tags=["posts"],)
...
@router.post(path="/",response_model=PostModel,status_code=201,summary="Создать пост",tags=["posts"],)
def post_create(post: PostCreate,user: User = Depends(get_current_user),post_service: PostService = Depends(get_post_service),) -> PostModel:
