...
class PostService(ServiceMixin):
    ...
    def create_post(self, user: User, post: PostCreate) -> dict:
        new_post = Post(title=post.title,description=post.description,owner_id=user.id)
...
@lru_cache()
def get_post_service(cache: AbstractCache = Depends(get_cache),session: Session = Depends(get_session),) -> PostService:
