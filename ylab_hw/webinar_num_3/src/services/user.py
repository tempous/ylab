...
from src.db.redis_db import (get_active_refresh_token,get_blocked_access_token)
from src.models.user import User
from src.services import ServiceMixin
from src.api.v1.schemas.auth import Login, Signup, Tokens
from src.core.config import (JWT_ACCESS_EXP_SECONDS,JWT_REFRESH_EXP_SECONDS,JWT_SECRET_KEY,JWT_ALGORITHM)
...
class UserService(ServiceMixin):
    ...
    def create_jwt(self, user):
        ...
        self.active_refresh_tokens.rpush(str(user.uuid), refresh_token_id)
    ...
    def login_user(self, login: Login):
    ...
    def refresh_token(self, refresh_jwt):
        decode_token = jwt.decode(refresh_jwt,JWT_SECRET_KEY,algorithms=[JWT_ALGORITHM])
        ...
        if token_id in active_tokens:
            ...
            user = self.session.query(User).filter(User.uuid == user_id).first()
            return self.create_jwt(user=user)
        else:
            raise ValueError

    def get_user(self, login: Login):
        user = self.session.query(User).filter(User.username == login.username).first()
        return user
    ...
    def logout(self, user, access_token):
        decode_access_token = jwt.decode(access_token,JWT_SECRET_KEY,algorithms=[JWT_ALGORITHM])
        self.blocked_access_tokens.set(decode_access_token.get('jti'), 1)
        token_uuids = []
        while self.active_refresh_tokens.llen(str(user.uuid)) != 0:
            token_uuid = self.active_refresh_tokens.rpop(str(user.uuid))
            if token_uuid != decode_access_token.get("refresh_token_jti"):
                token_uuids.append(token_uuid)
        if token_uuids:
            for i in range(0, len(token_uuids)):
                self.active_refresh_tokens.rpush(str(user.uuid), token_uuids[i])
...
def get_user_service(
        ...
        session: Session = Depends(get_session),
        blocked_access_tokens: Redis = Depends(get_blocked_access_token),
        active_refresh_tokens: Redis = Depends(get_active_refresh_token),) -> UserService:
    return UserService(
        cache=cache,
        session=session,
